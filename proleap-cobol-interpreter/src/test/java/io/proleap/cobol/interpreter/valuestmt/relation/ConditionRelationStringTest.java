...
@MicronautTest
public class ConditionRelationStringTest extends TestBase {

	@Test
	public void test() throws Exception {
		final File inputFile = new File(
				"src/test/resources/io/proleap/cobol/interpreter/valuestmt/relation/ConditionRelationString.cbl");
		...
		assertEquals("equal", state.getConsole().getLines().get(0));
		assertEquals("lesser equal", state.getConsole().getLines().get(1));
		assertEquals("greater equal", state.getConsole().getLines().get(2));
		assertEquals("lesser", state.getConsole().getLines().get(3));
		assertEquals("greater", state.getConsole().getLines().get(4));
	}
}
