package io.proleap.cobol.interpreter.procedure.string;
...
@MicronautTest
public class StringStatementTest extends TestBase {

	@Test
	public void test() throws Exception {
		final File inputFile = new File(
				"src/test/resources/io/proleap/cobol/interpreter/procedure/string/StringStatement.cbl");
		...
		assertEquals("WelcomeToTutorialspoint       ",
				valueService.getString(getValue("WS-STRING", programUnit, state)));
		assertEquals(BigDecimal.valueOf(25), valueService.getDecimal(getValue("WS-COUNT", programUnit, state)));
	}
}
