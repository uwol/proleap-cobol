...
@MicronautTest
public class ConditionEqualTest extends TestBase {

	@Test
	public void test() throws Exception {
		final File inputFile = new File(
				"src/test/resources/io/proleap/cobol/interpreter/valuestmt/relation/ConditionEqual.cbl");
		...
		assertEquals("same string", state.getConsole().getLines().get(0));
		assertEquals("string vs decimal", state.getConsole().getLines().get(1));

		assertEquals("same decimal", state.getConsole().getLines().get(2));
		assertEquals("same true boolean", state.getConsole().getLines().get(3));
		assertEquals("same false boolean", state.getConsole().getLines().get(4));

		assertEquals("same high-value", state.getConsole().getLines().get(5));
		assertEquals("same low-value", state.getConsole().getLines().get(6));
	}
}
