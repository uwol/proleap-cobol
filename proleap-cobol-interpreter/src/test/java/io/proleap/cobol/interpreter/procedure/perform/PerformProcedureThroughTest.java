...
@MicronautTest
public class PerformProcedureThroughTest extends TestBase {

	@Test
	public void test() throws Exception {
		final File inputFile = new File(
				"src/test/resources/io/proleap/cobol/interpreter/procedure/perform/PerformProcedureThrough.cbl");
		...
		assertEquals("Proc2", state.getConsole().getLines().get(0));
		assertEquals("Proc3", state.getConsole().getLines().get(1));
		assertEquals("Proc4", state.getConsole().getLines().get(2));
	}
}
