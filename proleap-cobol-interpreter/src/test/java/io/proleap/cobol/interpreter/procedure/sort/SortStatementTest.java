...
@MicronautTest
public class SortStatementTest extends TestBase {

	@Test
	public void test() throws Exception {
		final File inputFile = new File(
				"src/test/resources/io/proleap/cobol/interpreter/procedure/sort/SortStatement.cbl");
		...
		interpreterParams.setAcceptParams(Arrays.asList("7654321Obama   BO19610804", "1234567Hopper  GH19061209", " "));
		...
		assertEquals("Enter person details using template below.", state.getConsole().getLines().get(0));
		assertEquals("Enter - PersId, Lastname, Initials, YOB, MOB, DOB", state.getConsole().getLines().get(1));
